...
def parse_openfootball(data):
    ...
    for m in data.get("matches",[]):
        ...
        try:
            ...
            if h_score is not None: status="FINISHED"
            elif md==today:         status="LIVE"
            elif md<today:          status="FINISHED"
...
def merge_fd_into_matches(matches, fd_matches):
    ...
    for m in matches:
        ...
        if fd:
            ...
            if hs is not None: m["home_score"]=hs; m["away_score"]=as_
            if fd.get("status"): m["status"]=fd["status"]
...
def get_recent_matches(n=10):
    return sorted([m for m in get_cache()["matches"] if m["status"]=="FINISHED"],
