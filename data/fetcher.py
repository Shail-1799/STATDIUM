...
def get_flag_img(name, width=40, style=None):
    ...
    if style:
        s.update(style)
    if url:
        return html.Img(src=url, style=s, alt=name, title=name)
...
def parse_openfootball(data):
    ...
    for m in data.get("matches", []):
        ...
        group, venue, time_str = (
            m.get("group", m.get("round", "")),
            m.get("ground", ""),
            m.get("time", ""),
        )
...
def build_scorers_from_matches(matches):
    ...
    for m in matches:
        ...
        for g in m.get("goals1", []):
            n = g.get("name", "")
            if not n or "og" in n.lower():
                continue
            if n not in scorer_map:
                scorer_map[n] = {
                    "name": n,
                    "team": m["home_team"],
                    "goals": 0,
...
def fetch_fd_matches():
    ...
    try:
        r = requests.get(
            f"{FD_BASE}/competitions/{WC_ID}/matches", headers=FD_HEADERS, timeout=10
        )
        if r.status_code == 200:
...
def fetch_fd_scorers():
    ...
    try:
        r = requests.get(
            f"{FD_BASE}/competitions/{WC_ID}/scorers?limit=20",
            headers=FD_HEADERS,
            timeout=10,
        )
        if r.status_code == 200:
...
def fetch_fd_teams():
    ...
    for comp_id in [2000, "WC"]:
        try:
            r = requests.get(
                f"{FD_BASE}/competitions/{comp_id}/teams",
                headers=FD_HEADERS,
                timeout=10,
            )
            if r.status_code == 200:
                ...
                for t in teams:
                    ...
                    for our in all_our_teams:
                        our_l = our.lower()
                        if (
                            our_l in fd_name.lower()
                            or fd_name.lower() in our_l
                            or our_l in fd_short.lower()
                            or fd_short.lower() in our_l
                        ):
                            crest_map[our] = crest
                            break
                if crest_map:
    ...
    for team_name in all_our_teams[:16]:  # limit to avoid rate limit
        try:
            r = requests.get(
                f"{FD_BASE}/teams?name={requests.utils.quote(team_name)}",
                headers=FD_HEADERS,
                timeout=5,
            )
            if r.status_code == 200:
...
def merge_fd_into_matches(matches, fd_matches):
    ...
    for fd in fd_matches:
        home = _normalize_team_name((fd.get("homeTeam") or {}).get("name", ""))
        away = _normalize_team_name((fd.get("awayTeam") or {}).get("name", ""))
        if not home or not away:
            continue
        fd_lookup[(home, away)] = fd
    merged = []
    for m in matches:
        key = (
            _normalize_team_name(m["home_team"]),
            _normalize_team_name(m["away_team"]),
        )
        fd = fd_lookup.get(key)
        if fd:
            ft = fd.get("score", {}).get("fullTime", {})
            if ft.get("home") is not None:
                m["home_score"] = ft["home"]
                m["away_score"] = ft["away"]
            st = (fd.get("status") or "").upper()
            if st in {"FINISHED", "AET", "PEN", "AFTER_EXTRA_TIME", "AFTER_PENALTIES"}:
                m["status"] = "FINISHED"
            elif st in {"LIVE", "IN_PLAY", "PAUSED"}:
                m["status"] = "LIVE"
            elif m["status"] != "FINISHED" and st in {"TIMED", "SCHEDULED"}:
                m["status"] = "SCHEDULED"
...
def refresh_data():
    ...
    scorers = (
        ...
        if fd_scorers_raw
        else build_scorers_from_matches(matches)
    )
...
def get_recent_matches(n=10):
    terminal = {"FINISHED", "AET", "PEN", "AFTER_EXTRA_TIME", "AFTER_PENALTIES"}
    finished = [
        m for m in get_cache()["matches"] if (m.get("status") or "").upper() in terminal
    ]
