...
def layout():
    return html.Div([
        ...
        page_wrapper([html.Div(id="insights-content")]),
    ])
...
def update_insights(_):
    ...
    if scorers:
        for i,s in enumerate(scorers[:10],1):
            player=s.get("player",{}).get("name",s.get("name","–"))
            team=s.get("team",{}).get("shortName",s.get("team",{}).get("name","–"))
    ...
    for year, flag, team in WC_WINNERS:
        champions_html.append(html.Div([
            html.Span(flag,style={"fontSize":"20px","display":"block","textAlign":"center"}),
            html.Span(team,className="champ-team"),
            html.Span(year,className="champ-year"),
