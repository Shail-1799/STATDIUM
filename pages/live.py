...
@app.callback(Output("hero-stat-strip","children"), Input("live-interval","n_intervals"))
def update_hero_strip(_):
    ...
    def stat(val, lbl):
        return html.Div([
            html.Span(str(val), className="hero-stat-val countup-num",
...
def update_today(_):
    ...
    if not all_today:
        ...
        return html.Div([
            ...
            html.Div("First match: Mexico vs South Africa · Jun 11",
                     style={"fontSize":"13px","color":COLORS["text_secondary"],"textAlign":"center","marginTop":"6px"}),
        ], style={"padding":"32px 20px"})
    return _grouped_matches(all_today, "Today's Matches", f"{len(all_today)} match{'es' if len(all_today)!=1 else ''} today")


def _grouped_matches(matches, title, subtitle):
    """Group matches by date with styled date headers (Image 4 style)"""
    ...
    for date_key in sorted(by_date.keys()):
        ...
        date_groups.append(html.Div([
            # Date header
            html.Div([
                html.Span("•", style={"color":COLORS["accent"],"fontSize":"16px","fontWeight":"900","marginRight":"8px"}),
                html.Span(label, className="match-date-label"),
                html.Span(f"{len(day_matches)} match{'es' if len(day_matches)!=1 else ''}",
                          className="match-date-count"),
            ], className="match-date-header"),
            # Cards grid
            html.Div([match_scorecard(m) for m in day_matches], className="match-date-grid"),
